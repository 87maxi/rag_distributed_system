...
app = FastAPI(title="RAG Distributed API Gateway")
...
async def chat_completions(request: ChatCompletionRequest, x_api_key: Optional[str] = Header(None)):
    ...
    # Ejecutamos tu lógica de RAG (Busca en Qdrant -> Prompt -> LLM remoto)
    result = rag_system.ask(last_message)
    
    # Formateamos la respuesta para que parezca de OpenAI
    return {
        "id": "chatcmpl-rag",
